...
10. **Cormorant Garamond italic is the accent face.** The comp's Forward Serif is personal-use only and is not shipped; `--accent` in `amami.css` carries the licence note. Do not add Forward Serif unless a licence arrives.
...
| Instagram feed | footer, all pages | Six `.ftr__grid` stills → live six most-recent posts via Graph API once a token exists (`TODO(instagram)`). Keep them square, keep the frames. |
