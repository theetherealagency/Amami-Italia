...
T = {
    'en': dict(read_time='{n} min read', byline='By Amami Italia', kicker='Journal', sub='Notes on the cooking, the room and Brampton.', all='All', read='Read',
    ...
    'it': dict(read_time='{n} min di lettura', byline='Di Amami Italia', kicker='Giornale', sub='Appunti sulla cucina, sulla sala e su Brampton.', all='Tutti', read='Leggi',
...
def page(lang, en_url, it_url, title, desc, og_img, og_alt, og_type, main, jsonld=None, journal_post=False):
...
function fromHash(){{show(location.hash.slice(1));}}window.addEventListener('hashchange',fromHash);if(location.hash)fromHash();}})();</script>
        ...
        for p in posts:
            ...
            if p.get('layout') == 'journal':
                words = len(re.sub(r'<[^>]+>', ' ', L['body_html']).split())
                code = cats[p['category']].get('code')
                idx = (code + ' ' + cat) if code else cat
                main = f'''<main id="main">

<!-- Generated by tools/build-journal.py from content/journal/posts/{p['slug']}.json — edit the JSON, not this file.
     The client's Journal article design; styles in /_assets/amami-journal.css. -->
<article class="jr">
<header class="jr__head">
  <p class="jr__folio"><a class="idx" href="{url(None, lang)}#{p['category']}">{esc(idx)}</a><span>{t['read_time'].format(n=p.get('read_minutes') or max(1, round(words / 220)))}</span><span>{t['byline']}</span></p>
  <h1>{esc(L['title'])}</h1>
  {f'<p class="dek">{L["dek"]}</p>' if L.get('dek') else ''}
</header>
<figure class="jr__banner">{picture(p['photo']['base'], p['photo']['width'], p['photo']['height'], L['photo_alt'], '(max-width:720px) 100vw, 680px', 'jr__banner-img', True)}</figure>
{L['body_html']}
</article>
</main>'''
            else:
      ...
      <a class="pg-btn pg-btn--fill" href="{t['reserve_href']}" data-gtm="reserve">{t['reserve']}</a>
            ...
            write(url(p['slug'], lang).lstrip('/') + 'index.html',
                  page(lang, url(p['slug'], 'en'), url(p['slug'], 'it'), L['seo_title'], L['seo_description'],
                       og, L['photo_alt'], 'article', main, ld, journal_post=p.get('layout') == 'journal'))
